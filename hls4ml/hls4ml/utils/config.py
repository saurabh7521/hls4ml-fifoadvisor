    ...
    **kwargs,
    ...
    Args:
        ...
        version (str, optional): Optional string to version the generated project for backends that support it.
            Defaults to '1.0.0'.
        fifo_advisor (bool, optional): Enable automatic FIFOAdvisor execution after a Vitis build.
        fifo_advisor_config (dict, optional): Keyword arguments forwarded to FIFOAdvisor when it is enabled.
    ...
    if fifo_advisor is not None:
        config['FIFOAdvisor'] = fifo_advisor
    if fifo_advisor_config is not None:
        config['FIFOAdvisorConfig'] = copy.deepcopy(fifo_advisor_config)
