...
FIFOAdvisor Integration
=======================

For ``io_stream`` designs targeting the ``Vitis`` backend, this repository also supports automatically invoking
``fifo-advisor`` after ``hls_model.build()`` completes and the HLS solution directory has been generated.

This hook operates on ``<output_dir>/<project_name>_prj/solution1``. It does **not** target Vivado accelerator
``project_1`` block-design directories.
...
Custom FIFOAdvisor settings can be passed with ``fifo_advisor_config``:
...
``fifo_advisor`` is separate from hls4ml's built-in ``fifo_opt`` profiling flow. They should be treated as different
optimization mechanisms.

If both are enabled, hls4ml's internal FIFO profiling builds suppress ``fifo_advisor`` and FIFOAdvisor only runs once
after the final user-facing build completes.
